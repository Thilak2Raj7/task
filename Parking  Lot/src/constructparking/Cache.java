...
public class Cache {
	...
	Map<Long, Integer> customerReference = new HashMap<Long, Integer>();
...
//		}
	...
	public Map<Integer, Map<String, List<ParkingSpot>>> getSlotMap() {
		...
		String[] array = { "Compact", "Large", "Handicapped", "MotorCycle", "Electric vehicle" };
		for (int i = 0; i < checkIn.size(); i++) {
			Map<String, List<ParkingSpot>> obj = checkIn.get(i);

			for (String vehicle : array) {
				List<ParkingSpot> spotObj = obj.get(vehicle);
				System.out.println("Floor  " + i + "   Vehicle  " + vehicle + "" + "-" + spotObj.size());
			}
	...
	public void getFilledMap() {
		...
		for (Integer key : parked.keySet()) {
			...
			System.out.println(" Parking details for TokenNumber =" + 1);
	...
	public void getCustomerMap() {
		for (Integer key : customerInfo.keySet()) {
			CustomerInformation customer = customerInfo.get(key);
			System.out.println("Customer Information ");
			System.out.println("Name=" + customer.getName());
			System.out.println("Mobile number=" + customer.getMobileNumber());
			System.out.println("Customer Id=" + customer.getCustomerId());
			System.out.println("Wallet amount=" + customer.getWallet());
			System.out.println();
		}
	}
	...
	public void checkString(String name) throws Exception {
		if (name.isEmpty()) {
			throw new Exception("Name should be empty");
		}
	}
	...
	public void customerDetails(int tokenNumber) throws Exception {
		...
		scan.nextLine();
		int customerId = 0;
		if (customerReference.containsKey(mobileNumber)) {
			customerId = customerReference.get(mobileNumber);
			...
			customerReference.put(mobileNumber, customerId);
		...
		scan.nextLine();
		System.out.println("You can recharge your account through customer information portal ");
	...
	public void slotAllotment(String vehicleName, String vehicleNumber) throws Exception {
		...
		for (int i = 0; i < floorNumber; i++) {
			...
			if (!listObj.isEmpty()) {
				...
				System.out.println("Parking details for your vehicle :Token number=" + tokenNumber + "   spot number ="
						+ spotNumber + " in  ");
				if (i != 0) {
...
//		 System.out.println("Token Alloted-" +parked);
	...
	public boolean parkingPayment(int tokenNumber) throws Exception {
		float amount = 0;
		SlotAllotment slot = parked.get(tokenNumber);
		int customerId = slot.getCustomerId();
		...
		if (walletAmount >= 0) {
			...
			System.out
					.println("Enter the option to choose for your payment " + "Press 1 for Cash or Press  2 for Card");
