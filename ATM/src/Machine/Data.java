...
public class Data extends Thread {
	...
	public ATM checkAtmBalance() throws IOException {
	...
	public ATM readFile() throws IOException {
...
public void checkAccountNumber(long accountNumber) throws Exception
{
	...
	public void writeTransaction(long accountNo) throws IOException, InterruptedException {
		
		String customerId = String.valueOf(accountNo);
		try (BufferedWriter bw = new BufferedWriter(new FileWriter("<" + customerId + ">transaction.txt"))) {

			bw.write("TransactionNo" + "\t" + "Description" + "\t\t" + "Credit/debit " + "\t" + "Amount " + "\t"
					+ "ClosingBalance" + '\n');
			List<Transaction> trans = transaction.get(accountNo);
			...
			for (int i = 0; i < trans.size(); i++) {
				...
				bw.write(transaction + "\t\t" + description + "\t\t" + transferType + "\t\t" + amount + "\t\t"
						+ closingBalance + '\n');
	...
	public void transferLimit(int amount) throws Exception {
		...
		if (amount > 10000 || amount < 1000 || amount1 != 0) {


			
			throw new Exception("Maximum transfer limit  is 10000 && minimum transfer limit is 1000 && amount should be multiples of 100");
		}
	}
	...
	public void amountLimit(int amount) throws Exception {
		...
		if (amount > 10000 || amount < 100 || amount1 != 0) {
			throw new Exception(
					"Maximum withdraw limit is 10000 && Minimum withdraw limit is 100 && amount should be multiples of 100");
		}
	...
	public void readTransactionNumber() throws FileNotFoundException, IOException {
		try (BufferedReader bw = new BufferedReader(new FileReader("TransactionNumber.txt"))) {
			...
			while ((line = bw.readLine()) != null) {
				...
				line = line.trim();
				String trans = line;
				lastTransactionNo=Integer.parseInt(trans);
			};
		}
	}
	...
	public void transactionDetails(long sender, String describe, long receiver, long balance, int amount)
